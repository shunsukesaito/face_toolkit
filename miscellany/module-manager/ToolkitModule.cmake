if(${CMAKE_SYSTEM_NAME} MATCHES "Darwin")
  set (FACETOOLKIT_OS_SUFFIX "mac")
endif()
if(${CMAKE_SYSTEM_NAME} MATCHES "Linux")
  set (FACETOOLKIT_OS_SUFFIX "linux")
endif()
...
macro(FaceToolkitModuleBegin BASE_MODULE_NAME)
  if(TARGET FTM::${BASE_MODULE_NAME})
    return()
  endif()
  ...
  cmake_parse_arguments(FT_MODULE "" "" "${multiValueArgs}" ${ARGN} )

  # add every child modules first
  foreach(_module ${FT_MODULE_DEPENDS})
    findModule(${_module})
  endforeach(_module)
  ...
  if(FT_MODULE_LANGUAGES)
    project(${BASE_MODULE_NAME} LANGUAGES ${FT_MODULE_LANGUAGES})
  else()
    project(${BASE_MODULE_NAME})
  endif()
  include("${MODULE_MANAGER_ROOT}/ccache/setup.cmake")
  # here follows the actual code
endmacro(FaceToolkitModuleBegin)


macro(FaceToolkitModulePrebuilt BASE_MODULE_NAME)
  if(TARGET FTM::${BASE_MODULE_NAME})
    return()
  endif()
  set(_this_EXTERNAL_MODULES ${EXTERNAL_MODULES}) # backup
  ...
  if(NOT ACTUAL_BUILDING)
    ...
    elseif(NOT EXTERNAL_ONLY)
      ...
      ExternalProject_Add (${BASE_MODULE_NAME}
        ...
        CMAKE_ARGS -DACTUAL_BUILDING=ON "-DCMAKE_INSTALL_PREFIX=${MODULE_INSTALL_PATH}" "-DFaceToolkit_PLATFORM=${FaceToolkit_PLATFORM}"
  ...
  include("${MODULE_MANAGER_ROOT}/ccache/setup.cmake")
  # here follows the actual code
endmacro(FaceToolkitModulePrebuilt)


macro(FaceToolkitModuleEnd BASE_MODULE_NAME)
  ...
  add_library(FTM::${BASE_MODULE_NAME} ALIAS ${BASE_MODULE_NAME}) # attach FTM:: namespace for consistency when module is prebuilt

  install(TARGETS ${BASE_MODULE_NAME}
    EXPORT ${BASE_MODULE_NAME}Targets
    ARCHIVE DESTINATION lib/${FaceToolkit_OS_SUFFIX}
    LIBRARY DESTINATION lib/${FaceToolkit_OS_SUFFIX}
    INCLUDES DESTINATION include)
  #install(EXPORT ${BASE_MODULE_NAME}Targets DESTINATION cmake/${FaceToolkit_OS_SUFFIX})
  ...
  install(FILES ${CMAKE_CURRENT_BINARY_DIR}/${BASE_MODULE_NAME}Config.cmake
    DESTINATION cmake)
endmacro(FaceToolkitModuleEnd)

macro(FaceToolkitHeaderModule BASE_MODULE_NAME)
  if(TARGET FTM::${BASE_MODULE_NAME})
    return()
  endif()
  ...
  cmake_parse_arguments(FT_MODULE "${optionArgs}" "${singleValueArgs}" "${multiValueArgs}" ${ARGN} )
  if(NOT FT_MODULE_LOCATION)
    message(FATAL_ERROR "need to specify when header files are.")
  endif()
  FaceToolkitModuleBegin(${BASE_MODULE_NAME} DEPENDS ${FT_MODULE_DEPENDS})

  add_library(FTM::${BASE_MODULE_NAME} INTERFACE IMPORTED GLOBAL)
  set_target_properties(FTM::${BASE_MODULE_NAME} PROPERTIES
    INTERFACE_INCLUDE_DIRECTORIES "${FT_MODULE_LOCATION}"
  )

  if(NOT FT_MODULE_HIDE_HEADER)
...
endmacro(FaceToolkitHeaderModule)
