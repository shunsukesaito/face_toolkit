...
void LSRenderer::init(std::string data_dir, FaceModelPtr model)
{
    ...
    prog_main.createTexture("u_sample_mask", data_dir + "data/f2f_mask.png");
    ...
    {
        std::ifstream fin(data_dir + "data/LS/env_list.txt");
        if(fin.is_open()){
            while(!fin.eof())
            {
                std::string tmp;
    ...
    for(int i = 0; i < env_list.size(); ++i)
    {
         ...
         */
        std::string filename = data_dir + "data/LS/env/diff_" + env_list[i] + ".exr";
        ...
        filename = data_dir + "data/LS/env/ward0.15_" + env_list[i] + ".hdr.exr";
        ...
        filename = data_dir + "data/LS/env/ward0.37_" + env_list[i] + ".hdr.exr";
