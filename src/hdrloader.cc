...
static void flipVertical(float*src, float*dst, int width, int height);
...
void HDRLoader::flipVertical(HDRLoaderResult & img)
{
	float * dst = (float*)malloc(sizeof(float)*img.width*img.height * 3);
	for (int y = 0; y < img.height; y++)
	{
		for (int x = 0; x < img.width; x++)
		{
			dst[3 * (img.width*(img.height - 1 - y) + x) + 0] = img.cols[3 * (img.width*y + x) + 0];
			dst[3 * (img.width*(img.height - 1 - y) + x) + 1] = img.cols[3 * (img.width*y + x) + 1];
			dst[3 * (img.width*(img.height - 1 - y) + x) + 2] = img.cols[3 * (img.width*y + x) + 2];
		}
	}
    free(img.cols);
	img.cols = dst;
