...
#include <renderer/panorama_renderer.h>
...
struct Session{
    ...
    PanoramaRenderer renderer_;
    //XSlitCamera camera_;
    std::vector<XSlitCamera> cameras_;
...
void GUI::init(int w, int h)
{
    ...
    q1 << 0.f, 0.5f, 0.f;
    q2 << 0.f, -0.5f, 0.f;
    createCircleXSlits(3.0, q1, q2, 16, w, h, session.cameras_);

//    Eigen::Vector3f p1, p2, q1, q2;
//    p1 << -2.f, 0.f, 0.f;
//    p2 << 0.f, 0.f, -2.f;
//    q1 << 0.f, 1.f, 0.f;
//    q2 << 0.f, -1.f, 0.f;
//    session.cameras_.push_back(XSlitCamera(p1,p2,q1,q2,0.1f,2.f,512,512));
...
void GUI::loop()
{
    ...
    {        
        ...
        //session.renderer_.render(session.cameras_[0], session.data_.RT, session.data_.pts_, session.data_.nml_);
