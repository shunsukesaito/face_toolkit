...
#include <renderer/sphere_renderer.h>
...
//    session.renderer_.init(data_dir + "shaders", session.data_.tri_pts_);
    ...
    session.renderer_.init(data_dir, data_dir + "shaders");
...
void GUI::loop()
{
    ...
    lookat = Eigen::Vector3f::Zero();//Eigen::ApplyTransform(session.data_.RT,getCenter(session.data_.pts_));
    ...
    while(!glfwWindowShouldClose(session.windows_[MAIN]))
    {
        ...
        session.renderer_.render(session.camera_);
