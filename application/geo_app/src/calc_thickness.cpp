...
void GUI::init(int w, int h)
{
    ...
    std::string obj_path = FLAGS_data_dir + "/renderppl/" + FLAGS_subject_name + "_OBJ/" + FLAGS_subject_name + "_100k.OBJ";
    std::string dif_path = FLAGS_data_dir + "/renderppl/" + FLAGS_subject_name + "_OBJ/tex/" + FLAGS_subject_name + "_dif.jpg";
    std::string nml_path = FLAGS_data_dir + "/renderppl/" + FLAGS_subject_name + "_OBJ/tex/" + FLAGS_subject_name + "_norm.jpg";
    std::string out_path = FLAGS_data_dir + "/renderppl/" + FLAGS_subject_name + "_OBJ";
    ...
    // compute thickness
    session.obj_.computeThickness(900);
    session.obj_.setColorFromScalar(-session.obj_.thickness_, -0.02, 0.0);
    Eigen::write_to_text(out_path + "/thickness.txt", session.obj_.thickness_);
...
void GUI::loop()
{
    ...
    while(!glfwWindowShouldClose(session.windows_[MAIN]))
    {
        ...
        const auto& obj = session.obj_;
        session.geoRenderer_.render(session.camera_, obj.pts_, obj.nml_, obj.clr_, obj.RT_);
