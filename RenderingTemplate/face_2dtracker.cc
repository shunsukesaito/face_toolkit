...
DEFINE_string(npd_name, "1226model", "NPD face detection model");
DEFINE_string(spred_name, "shape_predictor_68_face_landmarks.dat", "Dlib shape predictor name");
...
Face2DDetector::Face2DDetector(std::string data_dir){
    ...
    dlib::deserialize(data_dir + "/data/" + FLAGS_spred_name) >> sp_;
    gab_detector_.LoadModel(data_dir + "/data/" + FLAGS_npd_name);
